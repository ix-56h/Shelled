...
/*   Updated: 2020/02/01 00:38:34 by niguinti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			init_and_post_process(char *s, t_gnt *gnt, t_lifo *stack)
{
	...
	if (!(gnt->chr_class = get_chr_class[(unsigned char)s[gnt->i]]))
	{
...
t_tokens	get_next_token(char *s, t_lifo *stack)
{
	...
	if (!(gnt.toktype = get_tok_type[gnt.chr_class]))
	...
	if (ABSTRACT_TOKEN[gnt.token.tok] && !(gnt.token.tok = \
